...
for( $i = $begin; $i <= $end; $i = $i + $beginrange )
{
	...
	if($visualchecked != "checked")
	{
		if(is_prime($areax[0]))
		{
			echo "<font face='arial,helvetica' color='#FFD801'>
			<strong>".$areax[0]."</strong></font>
			+ ( ( $i + 1 ) * 2 ) + ( ( ( $i * 2 ) - 1 ) * 2 ) = ";
		}
		else
		{
	...
	for($j = 0 ; $j < $jarray; $j++)
	{
		for($jcolor = 0; $jcolor < $jarray; $jcolor++)
			if($j == $jcolor)
			{
				...
				{
					if( ($j % 3) == 0)
						{ $color = "red"; $prexfix = " AX: ";}
					else if( ($j % 3) == 1)
						{ $color = "lightgreen"; $prexfix = " AY: ";}
					else if( ($j % 3) == 2)
						{ $color = "blue"; $prexfix = " AZ: ";}
		...
		if(is_prime($prime_check))
		{
			if($visualchecked == "checked")
			{
				...
				color="'.$color.'"><strong>
				<a title="'.$prime_check.' is a prime number." 
				alt="'.$prime_check.' is a prime number.">O</a></strong>
				</font>'; // O
				//**************************
				//**************************
			...
			else
			{
				...
				echo $prexfix.'<font face="arial,helvetica" size="2"
				color="'.$color.'"><strong> 
				<a title="'.$prime_check2.' is a prime number." 
				alt="'.$prime_check2.' is a prime number.">
				'.$prime_check.'</a>
				</strong> </font>';
				//**************************
				//**************************
...
$percentage = floor($superb / $c);
	...
	echo '<hr><br/><br/><strong> Total prime count : </strong>
	<font face="arial,helvetica" size="3"
	color="red"><strong>'.$superb.' / '.$endcount.'
	</strong></font> , %'.$percentage.' percentage, executed in '.$totaltime.' seconds
	<br/>';
